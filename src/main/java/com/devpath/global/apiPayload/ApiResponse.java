package com.devpath.global.apiPayload;
...
public class ApiResponse<T> extends ResponseEntity<ApiResponse.Body<T>> {
     ...
     * 성공적인 API 응답을 생성합니다.
     *
     * @param code   성공 코드와 메시지를 담고 있는 {@link BaseSuccessCode}
     * @param result 응답 데이터
     * @param <T>    응답 데이터의 타입
     ...
     * 실패한 API 응답을 생성합니다.
     *
     * @param code   에러 코드와 메시지를 담고 있는 {@link BaseErrorCode}
     ...
     * @return 성공 응답 {@link ApiResponse}
     */
    public static <T> ApiResponse<T> onSuccess(MultiValueMap<String, String> headers, BaseSuccessCode code, T result) {
        Body<T> body = Body.<T>builder()
                .isSuccess(true)
                .code(code.getCode())
                .message(code.getMessage())
     ...
     * @return 실패 응답 {@link ApiResponse}
     */
    public static <T> ApiResponse<T> onFailure(MultiValueMap<String, String> headers, BaseErrorCode code, T result) {
        Body<T> body = Body.<T>builder()
                .isSuccess(false)
                .code(code.getCode())
                .message(code.getMessage())
